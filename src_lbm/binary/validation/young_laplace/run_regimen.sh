...
for droplet_radius in "${droplet_radii[@]}"; do
  ...
  if [ -e "checkpoint_000200000" ]; then
    echo "Run complete"
    cd ..
    continue
  else
    ...
    sed -i "s/$search_pattern3/$replacement_text/g" inputs_young-laplace # edits input file with appropriate setting
    replacement_text="T = ${T}"
    sed -i "s/$search_pattern4/$replacement_text/g" inputs_young-laplace # edits input file with appropriate setting

    mpirun -n 2 $amrex_exec inputs_young-laplace > young_laplace.txt
    # EDIT COMMANDS HERE TO MAKE MODIFICATIONS TO RUNS #
  fi
  cd ..
done
